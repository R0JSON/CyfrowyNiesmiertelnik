...
import { Heart, Battery, Activity, Gauge, User, Radio, AlertTriangle, CheckSquare, Search, Clock } from 'lucide-react';
...
export function Sidebar({ tab, firefighters, beacons, alerts, onSelectFirefighter, selectedId, onAcknowledgeAlert }: Props) {
  ...
  if (tab === 'firefighters') {
    const filteredFirefighters = firefighters.filter(t => {
      const ff = t.firefighter;
      const term = searchTerm.toLowerCase();
      const battery = t.device.battery_percent.toString();
      
      return (
        ff.name.toLowerCase().includes(term) ||
        ff.id.toLowerCase().includes(term) ||
        ff.team?.toLowerCase().includes(term) ||
        ff.role.toLowerCase().includes(term) ||
        t.vitals.motion_state.toLowerCase().includes(term) ||
        battery.includes(term)
      );
    });

    return (
      <div className="flex flex-col h-full">
        <div className="p-3 border-b border-border bg-muted/20 sticky top-0 z-10">
          <div className="relative">
            <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
            <input
              type="text"
              placeholder="Szukaj (ID, imię, zespół...)"
              className="w-full bg-background border border-input rounded-md pl-8 pr-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground"
              value={searchTerm}
        ...
        <div className="divide-y divide-border overflow-y-auto">
          {filteredFirefighters.map(t => {
            const ff = t.firefighter;
            const vitals = t.vitals;
            const scba = t.scba;
            const device = t.device;
            const lastCommunicationTime = t.timestamp ? new Date(t.timestamp).toLocaleTimeString() : '-';
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{ff.team ? `${ff.team} · ` : ''}{ff.role}</div>
                    </div>
                  </div>
                  {/* Czas ostaniej komunikacji + ID strażaka */}
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-muted-foreground flex items-center gap-1" title="Ostatnia komunikacja">
  ...
  if (tab === 'alerts') {
    ...
    return (
      <div className="divide-y divide-border">
        {activeAlerts.map(a => (
          <div key={a.id} className="p-3 bg-psp-critical/10 border-l-4 border-l-psp-critical animate-pulse-critical">
