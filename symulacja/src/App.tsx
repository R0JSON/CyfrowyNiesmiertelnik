...
import { Users, Radio, AlertTriangle, Menu, ChevronLeft, Box, Map as MapIcon } from 'lucide-react';
...
export default function App() {
  ...
  const [showBeaconRanges, setShowBeaconRanges] = useState(false);
  const [showBeacons, setShowBeacons] = useState(false);
  const [viewMode, setViewMode] = useState<'2d' | '3d'>('2d'); // Nowy stan widoku

  // Pobieramy dane z zewnętrznego serwera
  const { firefighters, beacons, alerts, connected, sendCommand, resolveAlert, history, building } = useSimulationData();
        ...
        <header className="absolute top-4 left-4 right-4 z-[1000] flex justify-between pointer-events-none">
           <div className="flex gap-2 pointer-events-auto">
             ...
             <div className="bg-[#0d1117]/90 backdrop-blur border border-border rounded-lg p-2 flex gap-1 shadow-xl items-center">
                {/* Przełącznik widoku 2D/3D */}
                <div className="flex bg-muted rounded-md p-0.5 mr-2">
                  <button
                    onClick={() => setViewMode('2d')}
                    className={`p-1.5 rounded-sm transition-all ${viewMode === '2d' ? 'bg-background text-white shadow-sm' : 'text-muted-foreground hover:text-white'}`}
                    title="Widok 2D (Mapa)"
                  >
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-sm transition-all ${viewMode === '3d' ? 'bg-background text-white shadow-sm' : 'text-muted-foreground hover:text-white'}`}
                    title="Widok 3D (Izometryczny)"
                  >
                ...
                {[-1, 0, 1, 2].map(floor => (
                  <button
                    ...
                    className={`
                      ...
                      ${selectedFloor === floor && viewMode === '2d'
                        ? 'bg-psp-critical text-white' 
                        : 'bg-muted text-muted-foreground hover:bg-muted/80'}
                    `}
                    disabled={viewMode === '3d'} // Wyłącz wybór piętra w 3D (widać wszystkie)
                  >
                    {floor === -1 ? 'P' : floor}
                    {(floorCounts.get(floor) || 0) > 0 && (
                      <span className={`absolute -top-1 -right-1 w-4 h-4 text-[10px] rounded-full flex items-center justify-center ${selectedFloor === floor && viewMode === '2d' ? 'bg-white text-psp-critical' : 'bg-psp-critical text-white'}`}>
           ...
           {/* Panel kontrolny wizualizacji - tylko dla 2D */}
           {viewMode === '2d' && (
